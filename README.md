...
Например, есть задача — сделать экспорт некоего набора классов в XML. Дело было бы простым, если бы было можно редактировать классы узлов. Достаточно было бы добавить метод экспорта в каждый тип узла, а затем, перебирая узлы графа, вызывать этот метод для каждого узла. Благодаря полиморфизму, решение получилось бы изящным, так как не пришлось бы привязываться к конкретным классам узлов.

Но, к сожалению, классы узлов изменить не удалось. Код классов узлов сейчас очень стабилен, и от него многое зависит, поэтому никто не хочет рисковать и трогать его.

К тому же есть сомнения в том, что экспорт в XML вообще уместен в рамках этих классов. Их основная задача в другом, а экспорт выглядит в рамках этих классов чужеродно.

Была и ещё одна причина запрета. Если в скором времени понадобится экспорт в какой-то другой формат данных, эти классы снова придется менять.
