...
[ссылка на паттерн](https://refactoring.guru/ru/design-patterns/prototype)

**Назначение**: порождающий паттерн проектирования, который позволяет копировать объекты, не вдаваясь в подробности их реализации.
...
Паттерн Прототип реализован в базовой библиотеке TypeScript посредством интерфейса Cloneable.
...
[ссылка на паттерн](https://refactoring.guru/ru/design-patterns/singleton)

**Назначение**: порождающий паттерн проектирования, который гарантирует, что у класса есть только один экземпляр, и предоставляет к нему глобальную точку доступа.
...
Например одиночка может определить статический метод getInstance, который возвращает единственный экземпляр своего класса.
